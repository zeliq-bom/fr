...
void Ctask::lock_and_push(Ctask * task, MS_record & msr)
{
	...
	LOG("ts.%04d flags.%X x.%3f y.%3f z.%3f pit.%3f rol.%3f yaw.%3f  \n",
		msr.timestamp, msr.valid_flags, msr.x, msr.y, msr.z, msr.pitch, msr.roll, msr.yaw);
}
//-------------------------------------------------------------------------
	...
	{//----------------------------------Take lock MS
		...
		while (!temp_q.empty())
		{
			...
			LOG("PUSH to MS tid.%x ts.%04d flags.%X x.%3f y.%3f z.%3f pit.%3f rol.%3f yaw.%3f  \n",
				...
				temp.timestamp, temp.valid_flags, temp.x, temp.y, temp.z, temp.pitch, temp.roll, temp.yaw);
			accum_write = false;
			if (accum_active)
			{
				//------------check time flags
				...
				if (accum.timestamp = temp.timestamp)
				{
					...
					{
						accum.valid_flags |= temp.valid_flags;
						accum_write = false;
						LOG("PUSH to MS tid.%x combine records\n", std::this_thread::get_id());
						if (temp.valid_flags & VALID_X)
						{
							accum.x = temp.x;
						}
						if (temp.valid_flags & VALID_Y)
						{
							accum.y = temp.y;
						}
						if (temp.valid_flags & VALID_Z)
						{
							accum.z = temp.z;
						}
						if (temp.valid_flags & VALID_PITCH)
						{
							accum.pitch = temp.pitch;
						}
						if (temp.valid_flags & VALID_ROLL)
						{
							accum.roll = temp.roll;
						}
						if (temp.valid_flags & VALID_YAW)
						{
							accum.yaw = temp.yaw;
						}
					}
