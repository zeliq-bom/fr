...
class Ctask20 : public Ctask
{
	virtual void payloadA(MS_record & msr)
	{
		...
		msr.x = f(msr.timestamp);
	}
	virtual void payloadB(MS_record & msr)
	{
		msr.valid_flags = VALID_PITCH;
		msr.pitch = f(msr.timestamp)*M_PI;
	}
};
